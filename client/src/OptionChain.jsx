...
const OptionChain = ({ onClose }) => {
  ...
  const handleExecute = async () => {
    if (!window.confirm(`Execute ${selectedLegs.length} orders on Zerodha?`)) return;
    setIsExecuting(true);
    try {
      ...
      if (res.ok) { alert("Strategy Executed!"); setSelectedLegs([]); }
  ...
  return (
    <div className="h-screen bg-black text-gray-100 p-6 flex flex-col gap-6 overflow-hidden">
      <div className="flex justify-between items-center border-b border-gray-800 pb-4">
        <button onClick={onClose} className="bg-gray-800 px-4 py-2 rounded text-xs font-bold uppercase tracking-widest"><ArrowLeft size={14} className="inline mr-2" /> Back</button>
      ...
      <div className="flex-1 overflow-y-auto bg-[#0a0a0c] border border-gray-800 rounded-xl">
          ...
          <tbody>
            {chainData.map((row, i) => (
              <tr key={i} className="border-b border-gray-800/50 hover:bg-white/[0.02]">
                <td className="py-3"><ActionablePriceCell typeCEPE="CE" strike={row.strike} price={row.ce.ltp} onAddLeg={(t, s, o, p) => setSelectedLegs([...selectedLegs, { type: t, strike: s, optionType: o, price: p, qty: symbol === "NIFTY" ? 65 : 10 }])} selectedLegs={selectedLegs} /></td>
                <td className="text-center font-black text-lg bg-black/20 text-blue-400">{row.strike}</td>
                <td className="py-3"><ActionablePriceCell typeCEPE="PE" strike={row.strike} price={row.pe.ltp} onAddLeg={(t, s, o, p) => setSelectedLegs([...selectedLegs, { type: t, strike: s, optionType: o, price: p, qty: symbol === "NIFTY" ? 65 : 10 }])} selectedLegs={selectedLegs} /></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {selectedLegs.length > 0 && (
        <div className="p-4 bg-gray-900 border-t border-blue-500/30 rounded-t-2xl">
           <button onClick={handleExecute} disabled={isExecuting} className="w-full bg-blue-500 text-black py-4 rounded-xl font-black uppercase tracking-widest">{isExecuting ? "Executing..." : `Execute ${selectedLegs.length} Leg Strategy`}</button>
        </div>
      )}
    </div>
  );
